...
open class Chart: UIControl {
    ...
    */
    ...
    /** Minh Viet LE - Added leftInset
    Width of the area at the left of the chart, acting a padding to make place for the top x-axis label.
    */
    open var leftInset: CGFloat = 20
    ...
    */
    ...
    fileprivate func scaleValuesOnXAxis(_ values: [Double]) -> [Double] {
        ...
        let scaled = values.map { Double(self.leftInset) + factor * ($0 - self.min.x) }
        return scaled
    }
    ...
    fileprivate func drawLine(_ xValues: [Double], yValues: [Double], seriesIndex: Int) {
        // ------------------------------------------------------
        // Minh Viet LE - Add factorH to move line chart up
        // Get height factor
        var factorH: Double
        if max.y - min.y == 0 {
            factorH = 0
        } else {
            factorH = Double(drawingHeight) / (max.y - min.y)
        }
        ...
        path.move(to: CGPoint(x: CGFloat(xValues.first!), y: CGFloat(yValues.first! - factorH / 2)))
        for i in 1..<yValues.count {
            let y = yValues[i] - factorH / 2
    ...
    fileprivate func drawAxes() {
        ...
        // Minh Viet LE - Commented - Do not show vertical axis line
//        context.move(to: CGPoint(x: CGFloat(0), y: CGFloat(0)))
...
//        context.strokePath()
    ...
    fileprivate func drawLabelsAndGridOnXAxis() {
        ...
        scaled.enumerated().forEach { (i, value) in
            ...
            // Minh Viet LE - Commented - Do not add vertical lines
    ...
    fileprivate func drawLabelsAndGridOnYAxis() {
        ...
        scaled.enumerated().forEach { (i, value) in
            ...
            if y != drawingHeight + topInset && y != zero {
                ...
                // Minh Viet LE - Commented - Do not add dashed to any horizontal line
//                if labels[i] != 0 {
...
//            self.addSubview(label)
            // --------------------------------------------------------------
            let imageStrings = ["ic_awfulsm.png", "ic_badsm.png", "ic_btsm.png", "ic_goodsm.png", "ic_greatsm.png" ]
            ...
            let label = UIImageView(image: UIImage(named: imageStrings[[i, imageStrings.count - 1].min()!]))
            
            label.frame = CGRect(x: padding, y: y, width: 20, height: 20)
            ...
            // Labels should be placed above the horizontal grid
            label.frame.origin.y -= (label.frame.height + CGFloat(factorH)) / 2
